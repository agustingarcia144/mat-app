...
export default function PlanStatusCard({
  ...
  const remaining = isUnlimited ? Infinity : Math.max(0, weeklyLimit - weeklyUsed)
  const progressRatio = isUnlimited || weeklyLimit === 0 ? 0 : weeklyUsed / weeklyLimit
  ...
  return (
    ...
    >
      <View style={styles.header}>
        <Text
          style={[styles.planName, { color: isDark ? '#fff' : '#000' }]}
        >
          {plan.name}
        </Text>
        <View
