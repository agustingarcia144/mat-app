        ...
        <form onSubmit={submit} className="mx-auto max-w-3xl">
          <div className="relative rounded-xl border bg-background shadow-sm focus-within:ring-1 focus-within:ring-ring">
            <Textarea
              ...
              maxLength={4_000}
              className="min-h-20 resize-none border-0 pr-12 shadow-none focus-visible:ring-0"
              aria-label="Mensaje para Mati"
            />
            {busy ? (
              <Button type="button" size="icon" variant="ghost" className="absolute bottom-2 right-2" onClick={() => void stop()} aria-label="Detener respuesta">
                <CircleStop />
              </Button>
            ) : (
              <Button type="submit" size="icon" className="absolute bottom-2 right-2" disabled={!draft.trim() || exhausted} aria-label="Enviar mensaje">
                <ArrowUp />
              </Button>
            )}
          </div>
          <div className="mt-2 flex items-center justify-between gap-2 text-[11px] text-muted-foreground">
            <span>{usage.remaining} de {usage.limit} consultas disponibles</span>
            <span className="flex items-center gap-1"><Bot className="h-3 w-3" /> Los datos autorizados pueden ser procesados por OpenAI.</span>
          </div>
        </form>
      </div>
    </section>
