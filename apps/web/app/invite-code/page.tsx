...
function InviteCodeContent() {
  ...
  const redeemCode = useAction(api.orgCreationCodes.redeemCodeAndCreateOrganization)
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'No se pudo crear la organizacion')
    } finally {
    ...
    <main className="container mx-auto flex min-h-screen max-w-3xl items-center justify-center px-4 py-10">
      <Card className="w-full">
        <CardHeader>
          ...
          <CardDescription>
            Ingresa tu codigo para habilitar la creacion de una nueva organizacion.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          >
            ...
            {validationReason ? (
              <p className="text-sm text-destructive">{getReasonMessage(validationReason)}</p>
            ) : null}
            {validatedCode ? (
          ...
          {validatedCode && isLoaded && !userId ? (
            <div className="space-y-3 rounded-md border p-4">
              <p className="text-sm text-muted-foreground">
                El codigo es valido. Ahora inicia sesion para continuar con la creacion.
              </p>
          ...
          {validatedCode && userId ? (
            <form className="space-y-4 rounded-md border p-4" onSubmit={handleSubmit}>
              ...
              <Button type="submit" disabled={isSubmitting}>
                {isSubmitting ? 'Creando organizacion...' : 'Crear organizacion'}
              </Button>
            </form>
          ) : null}
        </CardContent>
