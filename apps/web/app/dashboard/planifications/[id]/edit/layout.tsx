...
function EditFormShell({
  ...
  const fullWeeksData = useMemo(() => {
    ...
    allExercises.forEach(
      (ex: Doc<"dayExercises"> & { exercise?: { name: string } | null }) => {
    ...
    const weeks = workoutWeeks.map((week: Doc<"workoutWeeks">) => {
      const daysForWeek = workoutDays.filter(
        (day: Doc<"workoutDays">) => day.weekId === week._id,
      );
      ...
      return {
        ...
        name: week.name,
        order: week.order,
        workoutDays: daysWithExercises.sort(
