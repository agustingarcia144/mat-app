...
function aggregateChartPoints(points: MetricPoint[], granularity: ChartGranularity) {
  if (granularity === 'day') {
...
export default function ExerciseMetricsPage() {
  ...
  const data = useQuery(api.metrics.getExerciseMetricsByMembers, canQuery ? {} : 'skip')
  ...
  useEffect(() => {
    ...
    const exists = selectedMember.planifications.some(
      (planification) => planification.planificationId === selectedPlanificationId
    )
  ...
  useEffect(() => {
    ...
    if (!visibleExercises.some((exercise) => exercise.exerciseId === selectedExerciseId)) {
              ...
              <div className='rounded-2xl border bg-card/70 p-4'>
                <div className='flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between'>
                  <div className='space-y-1'>
                    ...
                    <p className='text-sm text-muted-foreground'>
                      Ultima actividad: {formatDate(selectedMember.lastPerformedOn)}
                    </p>
                  </div>
                  ...
                  <div className='grid gap-3 md:grid-cols-2 xl:min-w-[520px]'>
                    <div className='space-y-2'>
                      ...
                      >
                        ...
                        <SelectContent>
                          {selectedMember.planifications.map((planification) => (
                            <SelectItem
              ...
              <div className='rounded-2xl border bg-card/70'>
                ...
                {visibleExercises.length === 0 ? (
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead className='min-w-[260px]'>Ejercicio</TableHead>
                    ...
                    <TableBody>
                      {visibleExercises.map((exercise) => {
                        const isOpen = exercise.exerciseId === selectedExerciseId
                        ...
                        return (
                          <Fragment key={exercise.exerciseId}>
                            ...
                            >
                              ...
                              {visibleColumns.includes('firstWeight') ? (
                                <TableCell>{formatMetricValue(exercise.firstWeight)}</TableCell>
                              ) : null}
                              {visibleColumns.includes('latestWeight') ? (
                                <TableCell>{formatMetricValue(exercise.latestWeight)}</TableCell>
                              ) : null}
                              {visibleColumns.includes('delta') ? (
                                <TableCell
                                  className={cn(
                              ...
                              {visibleColumns.includes('bestWeight') ? (
                                <TableCell>{formatMetricValue(exercise.bestWeight)}</TableCell>
                              ) : null}
                              {visibleColumns.includes('lastDate') ? (
                                <TableCell>{formatDate(exercise.lastPerformedOn)}</TableCell>
                              ) : null}
                            </TableRow>
                                  ...
                                  <div className='space-y-5 px-4 py-5'>
                                        ...
                                        <p
                                          className={cn(
                                            ...
                                            exercise.weightDelta !== null &&
                                              exercise.weightDelta < 0 &&
                                              'text-red-600',
                                            exercise.weightDelta === 0 && 'text-blue-600'
                                          )}
                                        >
                                          {formatDelta(exercise.weightDelta)}
                                        </p>
                                    ...
                                    <div className='space-y-3 rounded-2xl border bg-card/40 p-4'>
                                      <div className='flex flex-col gap-3 md:flex-row md:items-center md:justify-between'>
                                        <div>
                                          <p className='font-medium'>Evolucion del peso</p>
                                          <p className='text-sm text-muted-foreground'>
                                            El grafico resume los pesos usados segun la vista elegida.
                                          </p>
                                        </div>
                                          ...
                                          <TableBody>
                                            {exercise.points
                                              ...
                                              .map((point) => (
                                                ...
                                                >
                                                  <TableCell>
                                                    {formatDate(point.performedOn)}
                                                  </TableCell>
                                                  <TableCell>
                                                    {formatMetricValue(point.weight)}
                                                  </TableCell>
                                                </TableRow>
                                              ))}
                                          </TableBody>
