...
function AcceptInvitationContent() {
  ...
  const validateInvitationToken = useAction(
    api.organizations.validateInvitationToken,
  );
  const acceptInvitationByToken = useAction(
    api.organizations.acceptInvitationByToken,
  );
      ...
      .catch((error: unknown) => {
        ...
        toast.error(
          error instanceof Error
            ? error.message
            : "No se pudo validar la invitacion.",
        );
      })
      .finally(() => {
        ...
        <CardContent className="space-y-4">
          ...
          {!isValidating && validationReason ? (
            <div className="space-y-3 rounded-md border p-4">
              <p className="text-sm text-destructive">
                {getValidationMessage(validationReason)}
              </p>
          ...
          {!isValidating && !validationReason && invitation ? (
            <div className="space-y-4 rounded-md border p-4">
              <p className="text-sm">
                Fuiste invitado a <strong>{invitation.organizationName}</strong>{" "}
                como <strong>{invitation.roleLabel}</strong> para el email{" "}
