...
export default function NewRecurringRuleScreen() {
  ...
  const createRule = useMutation(api.finance.createRecurringRule);

  const [title, setTitle] = useState("");
  const [category, setCategory] = useState("");
  const [amountStr, setAmountStr] = useState("");
  const [dayOfMonth, setDayOfMonth] = useState(1);
  const [startPeriod] = useState(() => format(new Date(), "yyyy-MM"));
  ...
  const onSave = async () => {
    ...
    try {
      ...
      Alert.alert("Error", err?.message ?? "No se pudo crear la regla.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <ThemedView style={styles.container}>
      ...
      <View style={styles.safe}>
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          <View>
            <ThemedText type="defaultSemiBold" style={labelStyle}>
              Período de fin (opcional)
            </ThemedText>
            <TextInput
              style={inputStyle}
              value={endPeriod}
              onChangeText={setEndPeriod}
              placeholder="YYYY-MM (dejar vacío para sin fin)"
              placeholderTextColor={isDark ? Colors.dark.subtle : Colors.light.subtle}
              autoCapitalize="none"
            />
          </View>
