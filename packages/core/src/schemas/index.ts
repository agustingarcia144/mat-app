...
export const dayExerciseSchema = z
  .object({
    id: z.string(),
    exerciseId: z.string(),
...
const advancePaymentDiscountSchema = z.object({
  months: z.coerce.number().int().min(2, "Mínimo 2 meses"),
...
// Bonification schema
export const bonificationReasonOptions = [
  { value: "friend_and_family", label: "Familiar/Amigo" },
...
export const planBonificationSchema = z
  .object({
  ...
  .refine(
    (data) => {
      if (data.discountType === "full") return true;
      if (data.discountType === "percentage")
        return data.discountValue > 0 && data.discountValue <= 100;
      if (data.discountType === "fixed") return data.discountValue > 0;
      return false;
    },
    {
      message: "El valor del descuento es inválido",
      path: ["discountValue"],
    },
  );
...
export const financeRecurringExpenseSchema = z
  .object({
    ...
    dayOfMonth: z.coerce
      ...
      .min(1, "El día debe ser entre 1 y 28")
      .max(28, "El día debe ser entre 1 y 28"),
    startPeriod: financePeriodSchema,
    endPeriod: financePeriodSchema.optional().or(z.literal("")),
    paymentMethod: z
      .enum(["cash", "bank_transfer", "card", "other"])
      .optional(),
    notes: z.string().optional(),
  })
  .refine((data) => !data.endPeriod || data.endPeriod >= data.startPeriod, {
    message: "El período de fin debe ser igual o posterior al inicio",
    path: ["endPeriod"],
  });
