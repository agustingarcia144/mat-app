...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (duplicate) {
      throw new Error(
        "This member already has a fixed slot for this class, day and time",
      );
    }
...
async function assignFixedSlotToMatchingSchedules(
  ...
  for (const schedule of schedules) {
    ...
    const memberSub = await ctx.db
      ...
      .withIndex("by_organization_user", (q) =>
        q.eq("organizationId", slot.organizationId).eq("userId", slot.userId),
      )
 ...
 */
export function getDayAndMinutesInZone(
  startTime: number,
  timezone: string,
): { dayOfWeek: number; startTimeMinutes: number } {
