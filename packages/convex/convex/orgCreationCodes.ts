...
function getValidationReason(
  ...
  if (typeof code.expiresAt === "number" && code.expiresAt < now)
    return "expired";
  if (code.status === "consumed" || code.usedCount >= code.maxUses)
    return "consumed";
...
function getValidationMessage(
  reason: Exclude<CodeValidationReason, "invalid">,
): string {
...
export const redeemCodeAndCreateOrganization: ReturnType<typeof action> =
  action({
    args: {
      code: v.string(),
...
> = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const codeId = await ctx.runMutation(
      internal.orgCreationCodes.createOrgCreationCodeInternal,
      {
        codeHash,
...
export const issueOrgCreationCode: ReturnType<typeof internalAction> =
  internalAction({
    args: {
      maxUses: v.optional(v.number()),
