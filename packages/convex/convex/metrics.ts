...
export const getExerciseMetricsByMembers = query({
  ...
  handler: async (ctx) => {
    ...
    await Promise.all(
      Array.from(new Set(assignments.map((assignment) => assignment.userId))).map(
        async (userId) => {
    ...
    const organizationSessions = (await ctx.db.query('workoutDaySessions').collect()).filter(
      (session) => session.organizationId === organizationId && session.status !== 'skipped'
    )

    await Promise.all(
      Array.from(new Set(organizationSessions.map((session) => session.userId))).map(
        async (userId) => {
    ...
    for (const session of organizationSessions) {
      ...
      const name =
        user?.fullName || [user?.firstName, user?.lastName].filter(Boolean).join(' ') || userId
      ...
      let planification = planificationCache.get(String(session.planificationId))
      if (!planification) {
        planification = await ctx.db.get(session.planificationId)
        if (planification) {
          planificationCache.set(String(session.planificationId), planification)
        }
      }

      const planificationKey = String(session.planificationId)
      const planificationName = planification?.name ?? 'Plani sin nombre'
      const nextStatus = assignment?.status === 'active' ? 'active' : 'historical'
      const existingPlanification = memberEntry.planifications.get(planificationKey)
      ...
      if (!memberEntry.lastPerformedOn || session.performedOn > memberEntry.lastPerformedOn) {
      ...
      for (const log of logs) {
        ...
        const exerciseEntry =
          memberEntry.exercises.get(exerciseKey) ??
          {
        ...
        exerciseEntry.lastPerformedOn =
          !exerciseEntry.lastPerformedOn || session.performedOn > exerciseEntry.lastPerformedOn
            ? session.performedOn
            : exerciseEntry.lastPerformedOn
        exerciseEntry.planificationIds.add(planificationKey)

        const weight = parseLeadingNumber(log.weight ?? dayExercise.weight ?? null)
        ...
        const timeSeconds =
          ...
          (typeof dayExercise.timeSeconds === 'number'
            ...
            : null)
        const volume = weight !== null && reps !== null ? weight * reps * log.sets : null

        const previousPoint = exerciseEntry.pointsByDate.get(session.performedOn)
        const nextVolume = (previousPoint?.volume ?? 0) + (volume ?? 0)
        const nextTimeSeconds = (previousPoint?.timeSeconds ?? 0) + (timeSeconds ?? 0)

        exerciseEntry.pointsByDate.set(session.performedOn, {
          performedOn: session.performedOn,
          weight:
            previousPoint?.weight !== null && previousPoint?.weight !== undefined
    ...
    const serializedMembers = Array.from(members.values())
      .map((member) => {
        const exercises = Array.from(member.exercises.values())
          .map((exercise) => {
            const points = Array.from(exercise.pointsByDate.values()).sort((a, b) =>
            ...
            const baselineComparableValue = firstPoint?.weight ?? firstPoint?.volume ?? null
            ...
            if (
              ...
              else if (latestComparableValue < baselineComparableValue) trend = 'down'
            }
          ...
          .sort((a, b) => compareDatesDesc(a.lastPerformedOn ?? '', b.lastPerformedOn ?? ''))
        ...
        return {
          ...
          planifications: Array.from(member.planifications.values()).sort((a, b) => {
