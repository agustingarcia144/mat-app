...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (subscription.status === "cancelled") {
      throw new Error("No se puede bonificar una suscripci�n cancelada");
    }
    ...
    const existing = await ctx.db
      ...
      .first();
    if (existing) {
      throw new Error("Esta suscripci�n ya tiene una bonificaci�n activa");
    }
    ...
    if (currentPayment) {
      if (currentPayment.status !== "approved") {
        if (isFullBonification) {
          // Full bonification: mark as approved — user owes nothing
          ...
          // Partial bonification: update amount but keep pending — user still needs to pay
          await ctx.db.patch(currentPayment._id, {
...
export const revoke = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      ...
      throw new Error("Bonificaci�n no encontrada");
    }
    if (bonification.status === "revoked") {
      throw new Error("Esta bonificaci�n ya fue revocada");
    }
