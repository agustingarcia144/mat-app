...
export const startSession = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (assignment.organizationId !== organizationId) {
      throw new Error(
        "Access denied: assignment is outside the active organization",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Workout day does not belong to this assignment revision",
      );
    }
...
export const getMyWeekSessions = query({
  ...
  handler: async (ctx, args) => {
    ...
    const assignmentIds = Array.from(
      new Set(inRange.map((s) => s.assignmentId)),
    );
