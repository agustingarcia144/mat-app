...
export const getJoinPreview = action({
  ...
  handler: async (ctx, args): Promise<JoinPreviewResult> => {
    ...
    const { organizationId } = await ctx.runAction(
      internal.joinGymNode.verifyJoinToken,
      {
        token: args.token,
      },
    );
...
export const approveJoinRequest = action({
  ...
  handler: async (ctx, args) => {
    ...
    const membership = await ctx.runQuery(
      internal.joinGym.getMembershipByOrgAndUser,
      {
    ...
    await ctx.runMutation(
      internal.joinGym.createMembershipFromApprovedRequest,
      {
        organizationId: data.request.organizationId,
...
export const httpJoinPreview = httpAction(async (ctx, request) => {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ name: org.name, logoUrl: org.logoUrl }),
      {
        status: 200,
        headers: {
