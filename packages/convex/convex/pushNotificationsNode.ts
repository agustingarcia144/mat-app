...
export const sendExpoPushForEvent: ReturnType<typeof internalAction> =
  internalAction({
    args: {
      eventKey: v.string(),
      type: v.union(
    ...
    handler: async (ctx, args) => {
      ...
      if (activeTokens.length === 0) {
        await ctx.runMutation(
          internal.pushNotifications.markNotificationEventSkipped,
          {
            eventId: event.eventId,
      ...
      if (invalidTokens.length > 0) {
        await ctx.runMutation(
          internal.pushNotifications.deactivateTokensInternal,
          {
      ...
      if (successfulTickets > 0) {
        await ctx.runMutation(
          internal.pushNotifications.markNotificationEventSent,
          {
            eventId: event.eventId,
      ...
      await ctx.runMutation(
        internal.pushNotifications.markNotificationEventFailed,
        {
          eventId: event.eventId,
