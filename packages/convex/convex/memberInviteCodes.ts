...
import { requireAdminOrTrainer, requireCurrentOrganizationMembership } from './permissions'
...
export const createOrganizationMemberInviteCodeRecordInternal = internalMutation({
...
export const getActiveMemberInviteCodeByOrganizationInternal = internalQuery({
  ...
  handler: async (ctx, args): Promise<OrganizationMemberInviteCodeDetails | null> => {
...
export const getOrCreateOrganizationMemberInviteCode = action({
  ...
  handler: async (ctx): Promise<OrganizationMemberInviteCodeDetails> => {
    ...
    const existing = await ctx.runQuery(
      internal.memberInviteCodes.getActiveMemberInviteCodeByOrganizationInternal,
    ...
    while (attempts < 5) {
      ...
      const codeHash = await ctx.runAction(internal.memberInviteCodesNode.hashInviteCode, {
        code: normalizedCode,
      })
      ...
      const { token } = await ctx.runAction(internal.joinGymNode.createJoinToken, {
        organizationId: membership.organizationId,
      ...
      return await ctx.runMutation(
        internal.memberInviteCodes.createOrganizationMemberInviteCodeRecordInternal,
        {
          organizationId: membership.organizationId,
          code: normalizedCode,
          codeHash,
    ...
    throw new Error('No se pudo generar un código de invitación. Intenta nuevamente.')
  },
})
...
export const getOrganizationMemberInviteCodeDetails = query({
  ...
  handler: async (ctx) => {
    ...
    const active = await getActiveCodeByOrganization(ctx, membership.organizationId)
    if (!active) {
      return null
    }
