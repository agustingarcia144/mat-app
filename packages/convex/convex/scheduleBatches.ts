  ...
  handler: async (ctx, args) => {
    ...
    const hasStructuralChanges =
      batch.sourceConfig.mode === "timeWindow"
        ...
        : args.singleStartTime !== undefined ||
          args.durationMinutes !== undefined;
    ...
    if (hasStructuralChanges) {
      ...
      const protectedSchedules = schedules.filter(
        (schedule) =>
          !editableSchedules.some((editable) => editable._id === schedule._id),
      );
      ...
      if (batch.sourceConfig.mode === "timeWindow") {
        const config = {
          rangeStartDate:
            args.rangeStartDate ?? batch.sourceConfig.rangeStartDate,
          rangeEndDate: args.rangeEndDate ?? batch.sourceConfig.rangeEndDate,
          timeWindowStartMinutes:
            args.timeWindowStartMinutes ??
            batch.sourceConfig.timeWindowStartMinutes,
          timeWindowEndMinutes:
            args.timeWindowEndMinutes ??
            batch.sourceConfig.timeWindowEndMinutes,
          slotIntervalMinutes:
