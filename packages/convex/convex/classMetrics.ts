  ...
  handler: async (ctx, args) => {
    ...
    for (const reservation of reservations) {
      ...
      if (reservation.status === "attended") {
        ...
        stats.classCounts.set(
          classId,
          (stats.classCounts.get(classId) ?? 0) + 1,
        );
    ...
    const observedFrom =
      rangeDays > 0 ? rangeStart : (earliestConsidered ?? now);
    ...
    const dormantMembers = ranked
      ...
      .sort(
        (a, b) =>
          (b.daysSinceLastActivity ?? 0) - (a.daysSinceLastActivity ?? 0),
      )
      .slice(0, 10);
    ...
    return {
      ...
      overview: {
        ...
        avgPerWeek:
          activeMembers > 0
            ? Math.round(
                (totalActiveDays / activeMembers / observedWeeks) * 10,
              ) / 10
            : 0,
      },
      bySource,
